...
public final class McpProxyHandler implements HttpHandler
{
    ...
    // ------------------------------------------------------------------------------------
    // HTTP wire constants (mirrors fm.giper.edt.mcp.server.transport.McpHttpHandler /
    // fm.giper.edt.mcp.server.protocol.McpConstants — the proxy module has no compile
    ...
    /**
     * Hard cap on a buffered body's size: a body at or under this size is read in full; a bigger
     * one is rejected with {@code 413} - either up front (from a declared {@code Content-Length})
     * or as soon as the bounded read exceeds the cap - without ever buffering more than
     * {@code MAX_BODY_BYTES + 1} bytes, so an oversized or unbounded request body cannot exhaust
     * heap.
     *
     * <p>It governs EVERY read the proxy buffers, not only client requests: the same limit
     * applies to the backend responses this class and {@link BackendProfileChannel} have to hold
     * in memory. Backend responses that are merely RELAYED are streamed, never buffered, and are
     * not subject to it.</p>
     */
