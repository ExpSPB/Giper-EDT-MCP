...
4. **`kind="write-metadata"` tools get extra isolation** (the orchestrator does this, not you): immediately after such a test it waits for the fixture/export to become quiet, runs `reset_fixture()`, waits for EDT to consume the external file event, then uses `revalidate_objects()` plus `resync_to_disk(cleanDanglingReferences=true)` to refresh the model and remove only dangling references left by deleted test files. This avoids `clean_project`'s stop-time autosave on EDT 2025.2, which could write an unsaved model change back over the Git reset.
...
12. A `write-metadata` test that verifies the effect with **only** a MODEL READ-BACK, or **only** an on-disk diff, or `assert_no_diff()` alone — or one whose on-disk check asserts merely the **bare name** (false-matches the parent's collection reference) instead of the **structural element** (`<name>X</name>`, `<catalogs>Type.X</catalogs>`). Metadata writes persist to disk, and a call that submitted its own export also drains that queue before answering (§3 - `rename_metadata_object` / `build_external_objects` do neither), so verify BOTH: model read-back AND an assertion on the specific on-disk element - direct when the call submitted its own export, polled when it did not (for the generic delete: its container's file is immediate, the deleted object's own file is polled). For delete/rename, also: a confirm test that doesn't read back the gone/renamed object, or a preview test that doesn't assert the object is UNCHANGED.
...
def test_create_attribute_appears_in_model_and_on_disk():
    ...
    # DISK FIRST: create submits its own export, so the bytes are settled the moment the call
    # returns - but only until the next MCP round trip gives a slower build time to catch up.
    assert_diff_contains("<name>%s</name>" % new_attr, "ON-DISK: the new attribute reaches the .mdo")
...
- [ ] Correct `kind=` chosen, and the effect is verified the RIGHT way: **on-disk diff** (file-write) / **model read-back + a direct on-disk assertion** (write-metadata) / **`assert_no_diff()`** (read) / **env-robust sentinel branch** (env-dependent). Never `assert_no_diff()` as the only assertion.
