...
public class GetServerStatusToolTest
{
    ...
    private static void assertEffectiveState(JsonObject flags, String key)
    {
        ...
        assertTrue(key + " atStartup must be a string", //$NON-NLS-1$
            state.get("atStartup").getAsJsonPrimitive().isString()); //$NON-NLS-1$
        String atStartup = state.get("atStartup").getAsString(); //$NON-NLS-1$
        boolean valid = "on".equals(atStartup) || "off".equals(atStartup) //$NON-NLS-1$ //$NON-NLS-2$
            || "unknown".equals(atStartup); //$NON-NLS-1$
        assertTrue(key + " atStartup must be on, off, or unknown", valid); //$NON-NLS-1$
        assertFalse(key + " must not publish the removed effective key", state.has("effective")); //$NON-NLS-1$ //$NON-NLS-2$
    }
