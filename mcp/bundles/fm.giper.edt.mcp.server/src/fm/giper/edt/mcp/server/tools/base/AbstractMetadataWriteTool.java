    ...
    private ProjectContext resolveProjectRoot(String projectName, boolean allowNoConfiguration)
    {
        ...
        if (project == null || !project.exists())
        {
            ...
            ctx.error = ToolResult.error(
                fm.giper.edt.mcp.server.utils.ProjectContext.notFoundMessage(projectName)).toJson();
            return ctx;
        }
        ...
        {
            ctx.error = ToolResult.error(
                fm.giper.edt.mcp.server.utils.ProjectContext.unreadableExternalRootMessage(
                    projectName)).toJson();
            return ctx;
        }
        ...
        {
            ctx.error = ToolResult.error(
                fm.giper.edt.mcp.server.utils.ProjectContext.noConfigurationMessage(
