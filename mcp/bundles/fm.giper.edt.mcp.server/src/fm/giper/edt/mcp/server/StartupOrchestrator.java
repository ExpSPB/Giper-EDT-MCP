...
import fm.giper.edt.mcp.server.groups.IGroupService;
import fm.giper.edt.mcp.server.groups.internal.GroupServiceImpl;
import fm.giper.edt.mcp.server.utils.WorkmateChatSessionPublisher;
...
public class StartupOrchestrator
{
    ...
    public void start(boolean headless)
    {
        ...
        if (!headless)
        {
            ...
            fm.giper.edt.mcp.server.tags.ui.FilterByTagManager.getInstance();
            ...
            org.eclipse.swt.widgets.Display.getDefault().asyncExec(() -> {
                ...
                try
                {
                    fm.giper.edt.mcp.server.groups.ui.NavigatorEnhancementManager
                ...
                try
                {
                    fm.giper.edt.mcp.server.ui.NavigatorToolbarCustomizer.getInstance().initialize();
                }
                catch (Exception e)
                {
    ...
    public void stop(boolean headless)
    {
        ...
        if (!headless)
        {
            ...
            if (display != null && !display.isDisposed())
            {
                try
                {
                    fm.giper.edt.mcp.server.ui.NavigatorToolbarCustomizer.getInstance().dispose();
                }
                catch (Exception e)
                {
                    // Ignore - workbench may be closing
